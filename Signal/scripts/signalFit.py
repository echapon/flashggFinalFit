...
nominalWSFileName = glob.glob("%s/output_%s_%s.root"%(opt.inputWSDir,opt.proc,MHNominal))[0]
...
if opt.skipZeroes:
  ...
  WSFileName = glob.glob("%s/output_%s_%s.root"%(opt.inputWSDir,opt.proc,MHNominal))[0]
...
for mp in opt.massPoints.split(","):
  ...
  WSFileName = glob.glob("%s/output_%s_%s.root"%(opt.inputWSDir,procRVFit,mp))[0]
  f = ROOT.TFile(WSFileName,"read")
  inputWS = f.Get(inputWSName__)
...
if( datasetRVForFit[MHNominal].numEntries() < opt.replacementThreshold  )|( datasetRVForFit[MHNominal].sumEntries() < 0. ):
  ...
  for mp in opt.massPoints.split(","):
    ...
    WSFileName = glob.glob("%s/output_%s_%s.root"%(opt.inputWSDir,procReplacementFit,mp))[0]
    f = ROOT.TFile(WSFileName,"read")
    inputWS = f.Get(inputWSName__)
...
if not opt.skipVertexScenarioSplit:
  ...
  for mp in opt.massPoints.split(","):
    ...
    WSFileName = glob.glob("%s/output_%s_%s.root"%(opt.inputWSDir,procWVFit,mp))[0]
    f = ROOT.TFile(WSFileName,"read")
    inputWS = f.Get(inputWSName__)
